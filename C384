...
class LiveTrader:
    ...
    def start(self, params, model):
        ...
        self.data_history = TechnicalAnalysis.extract_features(initial_data, self.params.get('FUTURE_WINDOW', 1), is_live_trading=True)        if self.data_history.empty:
            self._log("❌ Initial data fetch failed. Cannot start live trading.")
            self.running = False
            return
    ...
    def _live_trading_loop(self):
        try:
            while self.running:
                ...
                new_candles = data[data.index > self._last_candle_timestamp]

                if not new_candles.empty:
                    self._log(f"🔄 Found {len(new_candles)} new candles.")

                    self.data_history = pd.concat([self.data_history, new_candles])

                    # IMPORTANT: Re-extract features on the combined data to ensure all features are correctly calculated, especially rolling ones.
                    self.data_history = TechnicalAnalysis.extract_features(self.data_history, self.params.get('FUTURE_WINDOW', 1), is_live_trading=True)

                    # CORRECTED LOGIC: Check if there's enough data after feature extraction
                    if self.data_history.empty or len(self.data_history) < 2:
                        self._log("❌ Insufficient data after feature extraction. Skipping.")
                        continue

                    # The error 'single positional indexer is out-of-bounds' happens here if the DataFrame is too small.
                    # The check above prevents this. The code will now safely access the last element.
                    latest_candle = self.data_history.iloc[-1]
                    self._process_single_candle(latest_candle)
                    self._last_candle_timestamp = new_candles.index[-1]
    ...
    def get_status(self):
        open_trade = next((t for t in self.results['trades'] if t.get('status') == 'OPEN'), None)
        closed_trades = [t for t in self.results['trades'] if t.get('status') == 'CLOSED']        latest_price = self.data_history['Close'].iloc[-1] if not self.data_history.empty else 0
    ...
    try:
        ...
        with zipfile.ZipFile(zip_buffer, 'w', zipfile.ZIP_DEFLATED, compresslevel=6) as zip_file:
            ...
            for file in os.listdir('.'):
                if (file.endswith('.py') and file not in exclude_files and file != 'main.py' and os.path.isfile(file)):
